...
export default async function farm(bot: Bot, seedType: Item, moveHandler: MoveHandler, start: Vec3, length: number, width: number, rows: number, input: Vec3) {
    const FAR = 1
    const CLOSE = .1
    let a = 1;
    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < length + 1; j++) {
            await moveHandler.moveTo(bot, start, FAR, CLOSE)
            let pos = start.clone()
                .subtract(new Vec3(-Math.floor(width / 2), 0, 0))
            for (let k = 0; k < width; k++) {
                if (bot.heldItem === null) {
                    if (bot.inventory.findInventoryItem(seedType.id, null, false) === null) {
                        if (!await getAndWithdraw(moveHandler, bot, input, seedType, INGREDIENT_AMOUNT)) {
                    ...
                    if (bot.heldItem === null // bot#equip errors when hand is already full
                        ...
                        try {
                            ...
                            if (e instanceof Error) {
                                throw new Error('Error while equipping ' + seedType.id + '. ' + e.message + ', ' + bot.inventory.items().toString())
                            } else {
                                throw e
                            }
                        }
